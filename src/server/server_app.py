...
def run_server() -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            # 1) Send server_hello with Kyber public key + session params
            send_frame(conn, {
                "type": "server_hello",
                "pk_b64": b64e(pk),
            ...
            # 2) Receive client_kem (ct + client nonce prefix)
            msg = recv_frame(conn)
            ...
            # 3) Decapsulate to obtain shared secret
            shared_secret = kyber512.decrypt(ct, sk)
            ...
            # Basic nonce check (prefix + monotonic counter expectation)
